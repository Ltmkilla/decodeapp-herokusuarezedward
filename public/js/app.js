...
class Component extends App{
  ...
  viewSuits(suitid){
    ...
    <button onclick="component.updateSuits(${r.suitid})">Update Blackshop</span>
    </div>            
    </div>     
    </div>   
...
updateSuitApp(suitid){

  suitid = suitid+1;
      let suitDummy = {
        "suitid" :  suitid,       
        "suitname" : $('#suitname1').val(),
        "suitpic" : $('#suitimg').val(),
        "suitdesc" : $('#suitdes').val()
        
      }

    let r = this.suits;
    for(let i=0;i<r.length;i++){
      if(r[i].suitid == suitid){
        r[i] = suitDummy;
        break;
      }
    }
...
<form>
    <div class="row">
      <div class="input-field col s6">
        <h5><B>Updated Suites:</B></h5>
        <tr>
        <input id="suitname1" type="text" class="validate" value="${this.suits[suitid].suitname}"></div>
         <input id="suitimg" type="text" class="validate" value="${this.suits[suitid].suitpic}"></div>
          <div class="input-field col s6">
            <h5><B>Suit Details:</B></h5>

           
            <textarea id="suitdes" type="text" class="textarea-field" value="${this.suits[suitid].suitdesc}"></textarea>
    </div>
      <a onclick="component.updateSuitApp(${suitid})" class="waves-effect blue waves-light btn">Update</a>
</form>
`;  
    this.reRender(`
    ...
    `,document.getElementById("updateSuits"));   
